...
const initWorld = () => {
    ...
    for (let i = 0; i < totalSpots; i++) {
        ...
        newSpots.push({
            ...
            rect: {
                x: xPos,
                y: isTop ? 0 : 170, // Ajusté pour le layout
                w: 90,
                h: 130
            }
        });
...
const updatePhysics = () => {
    ...
    // Collisions Murs
    if (car.value.y < -50) { car.value.y = -50; car.value.speed *= -0.5; }
    if (car.value.y > 270) { car.value.y = 270; car.value.speed *= -0.5; }
    if (car.value.x < 0) { car.value.x = 0; car.value.speed = 0; }
    if (car.value.x > worldWidth.value - 50) { car.value.x = worldWidth.value - 50; car.value.speed = 0; }
                    ...
                    <div class="world" :style="{ width: worldWidth + 'px', transform: `translateX(${-cameraX}px)` }">
                        <div class="road">
                            <div v-for="spot in spots" :key="spot.id"
                                 class="parking-spot"
                                 :class="[spot.side === 'top' ? 'spot-top' : 'spot-bottom', { 'valid-zone': isCarInSpot(spot) }]"
                                 :style="{ left: spot.x + 'px' }">
                                <span>{{ spot.user }}</span>
                            </div>
                            <div class="car" :style="carStyle">
                                <div class="lights"><div class="light l"></div><div class="light r"></div></div>
                            </div>
                        </div>
                    </div>
                </div>
...
.fake-input:hover { border-color: #333; background: #fff; }
.fake-input.filled { border-color: #2ecc71; background: #eafaf1; color: #27ae60; font-weight: bold; }
...
.hud {
    ...
    background: rgba(0,0,0,0.8); color: white; padding: 10px 30px; border-radius: 30px;
    text-align: center; border: 1px solid #555; z-index: 200;
}
.quit-btn { margin-top:5px; background:none; border:1px solid #aaa; color:#aaa; padding:2px 8px; border-radius:4px; cursor:pointer; }
.game-container { flex: 1; overflow: hidden; position: relative; background: #2c3e50; }
.world { height: 100%; position: absolute; top: 0; left: 0; background-color: #3a5a40; display: flex; align-items: center; }
.road {
    height: 300px; background: #34495e; position: relative;
    border-top: 10px solid #bdc3c7; border-bottom: 10px solid #bdc3c7; box-sizing: content-box;
}
.road::after {
    content: ''; position: absolute; top: 50%; left: 0; right: 0; height: 2px;
    background: linear-gradient(90deg, #fff 50%, transparent 50%);
    background-size: 80px 100%; transform: translateY(-50%); opacity: 0.5;
}
.parking-spot {
    position: absolute; width: 90px; height: 130px;
    border: 2px solid rgba(255,255,255,0.6); box-sizing: border-box;
    display: flex; justify-content: center; align-items: flex-end;
    color: rgba(255,255,255,0.8); font-weight: bold; font-size: 12px; padding-bottom: 10px;
}
.spot-top { top: 0; border-top: none; }
.spot-top span { transform: rotate(180deg); margin-bottom: 80px; }
.spot-bottom { bottom: 0; border-bottom: none; }
.valid-zone { background: rgba(46, 204, 113, 0.2); border-color: #2ecc71; }
.car {
    width: 44px; height: 80px; background: #e74c3c; border-radius: 6px;
    position: absolute; top: 0; left: 0; transform-origin: center center;
    box-shadow: 0 10px 20px rgba(0,0,0,0.5); z-index: 10;
}
.car::after { content: ''; position: absolute; top: 15px; left: 4px; right: 4px; bottom: 10px; background: #c0392b; border-radius: 4px; }
.car::before { content: ''; position: absolute; top: 20px; left: 6px; right: 6px; height: 15px; background: #333; border-radius: 2px; z-index: 1; }
.lights { position: absolute; top: 0; width: 100%; height: 10px; }
.light { width: 8px; height: 12px; background: #f1c40f; border-radius: 4px; position: absolute; top: -2px; box-shadow: 0 0 10px #f1c40f; }
.light.l { left: 4px; } .light.r { right: 4px; }
.message-toast {
    position: absolute; bottom: 100px; left: 50%; transform: translateX(-50%);
    background: #e74c3c; color: white; padding: 10px 20px; border-radius: 5px; font-weight: bold;
}
.pop-enter-active, .pop-leave-active { transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
.pop-enter-from, .pop-leave-to { transform: translateX(-50%) scale(0.5); opacity: 0; }
</style>